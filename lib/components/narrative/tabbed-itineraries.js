...
class TabbedItineraries extends Component {
  ...
  render() {
    const {
      ...
      setActiveItinerary,
      showCalories,
      showPathNotFoundErrorWithItins,
      timeFormat,
      ...itineraryBodyProps
    } = this.props
      ...
      <div className="options itinerary tabbed-itineraries">
        {error &&
          ...
          (showPathNotFoundErrorWithItins
            ? true
            : !(
                error.message === 'PATH_NOT_FOUND' && itineraries.length > 0
...
const mapStateToProps = (state) => {
  ...
  const showCalories = state.otp.config.itineraries?.showCalories || false
  const pending = activeSearch ? Boolean(activeSearch.pending) : false
  const realtimeEffects = getRealtimeEffects(state)
  const showPathNotFoundErrorWithItins =
    state.otp.config.itinerary?.showPathNotFoundErrorWithItins !== false
