...
function constructRoutingQuery (otpState, ignoreRealtimeUpdates) {
  ...
  return `${planEndpoint}?${stringParams}&apikey=${otpApiKey}`
}
...
function createQueryAction (endpoint, responseAction, errorAction, rewritePayload, postprocess) {
  return async function (dispatch, getState) {
    ...
    // console.log("createQueryAction endpoint")
    // console.log(endpoint)
    //
    // console.log("createQueryAction api")
    // console.log(api)

    const otpApiKey = api.otpApiKey

    // console.log("createQueryAction otpApiKey")
    // console.log(otpApiKey)

    let url = `${api.host}${api.port ? ':' + api.port : ''}${api.path}/${endpoint}`
    if (endpoint.includes("?")) {
       url = url+`&apikey=${otpApiKey}`
    }else{
      url = url+`?apikey=${otpApiKey}`
    }
