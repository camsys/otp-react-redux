...
class LocationField extends Component {
  ...
  _geocodeAutocomplete (text) {
    ...
    this._geocodeAddressWithGoogle(text)

    // Mapzen is no longer supported.
    /*
  ...
  _geocodeAddressWithGoogle (address) {
    
    var outerThis = this;
    const boundary = this.props.config.geocoder.boundary;
    var bounds = new google.maps.LatLngBounds(
               new google.maps.LatLng(boundary.rect.minLat, boundary.rect.minLon), 
               new google.maps.LatLng(boundary.rect.maxLat, boundary.rect.maxLon));
    this.geocoder.geocode({ 'address': address, 'bounds': bounds }, function handleResults(results, status) {

      console.log(results)

      if (status === google.maps.GeocoderStatus.OK) {

        outerThis.setState({ geocodedFeatures: results })
      }
    ...
    if (geocodedFeatures.length > 0) {
      ...
      menuItems = menuItems.concat(geocodedFeatures.map(feature => {
        ...
        const locationSelected = () => {
          ...
          //const location = lonlat.fromCoordinates(feature.geometry.coordinates) // Mapzen
          const location = lonlat.fromCoordinates([feature.geometry.location.lng(), feature.geometry.location.lat()]) // Google
          location.name = feature.formatted_address // Google
          //location.name = feature.properties.label // Mapzen
        ...
        const option = createOption('map-pin', feature.formatted_address, locationSelected, itemIndex === activeIndex) // Google
        //const option = createOption('map-pin', feature.properties.label, locationSelected, itemIndex === activeIndex) // Mapzen
        itemIndex++
        return option
      }))
    }
